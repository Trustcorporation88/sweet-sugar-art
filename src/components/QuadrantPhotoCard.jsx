...
const QuadrantPhotoCard = ({ product }) => {
  ...
  const isGourmet = product.category === 'Gourmet';

  // Gourmet exibe apenas a primeira foto, enquadrada; Personalizados mantém o grid de 4
  const photos = isGourmet
    ? [urls[0] || null]
    : [urls[0] || null, urls[1] || null, urls[2] || null, urls[3] || null];
  ...
  const renderImage = (imgSrc, index, single = false) => (
    <div
      key={index}
      className={`relative overflow-hidden bg-[#E8E4E0] ${imgSrc ? 'cursor-pointer' : ''} ${
        single ? 'aspect-square w-full' : 'aspect-square'
      }`}
    ...
    >
      {imgSrc ? (
        <>
          <SafeImage
            src={imgSrc}
            alt={`${product.name}${single ? '' : ` - view ${index + 1}`}`}
  ...
  return (
    <>
      <div className="group flex flex-col h-full bg-[#F5F1ED] rounded-sm overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-[#8B6F47]/10">
        {isGourmet ? (
          <div className="aspect-square w-full bg-white border-b border-[#8B6F47]/20 p-2 flex items-center justify-center">
            <div className="relative w-full h-full border-4 border-[#8B6F47]/20 p-1 bg-[#F5F1ED]">
              {renderImage(photos[0], 0, true)}
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-2 aspect-square w-full gap-[3px] bg-white border-b border-[#8B6F47]/20 p-[3px]">
            {photos.map((imgSrc, index) => renderImage(imgSrc, index))}
          </div>
        )}
